...
def main(page):
    ...
    for index, row in df.iterrows():
        ref_location = row['参考位置']
        province = row['省份']

        # 如果省份为非国内和台湾、北京、上海、重庆、天津、香港、澳门，则不进行筛选，直接填充
        if province == '非国内':
            df.at[index, '城市'] = f"非国内"
            continue
        if province == '台湾':
            df.at[index, '城市'] = f"台湾"
            continue
        ...
        for sep in separators:
            if sep in ref_location:
                parts = ref_location.split(sep)
                if len(parts) > 1:
                    area = parts[0].strip()[2:]
                    df.at[index, '城市'] = f"{area}"
                    found_sep = True
                    break
        if not found_sep:
            df.at[index, '城市'] = f"未知区域"
